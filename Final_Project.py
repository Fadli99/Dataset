...
def create_sum_category_name_df(df):
    sum_category_name_df = df.groupby("product_category_name").order_id.nunique().sort_values(ascending=False).reset_index()
    return sum_category_name_df
...
sum_category_name_df = create_sum_category_name_df(main_df)
...
st.subheader("Best & Worst Performing City Order Product")
...
ax[0].set_title("Best Performing City Order Product", loc="center", fontsize=50)
...
ax[1].set_title("Worst Performing City Order Product", loc="center", fontsize=50)
...
sns.barplot(x="order_id", y="product_category_name", data=sum_category_name_df.head(5), palette=colors, ax=ax[0])
ax[0].set_ylabel(None)
ax[0].set_xlabel("Number of Category", fontsize=30)
ax[0].set_title("Best Performing Product Category", loc="center", fontsize=50)
ax[0].tick_params(axis='y', labelsize=35)
ax[0].tick_params(axis='x', labelsize=30)
 
sns.barplot(x="order_id", y="product_category_name", data=sum_category_name_df.sort_values(by="order_id", ascending=True).head(5), palette=colors, ax=ax[1])
ax[1].set_ylabel(None)
ax[1].set_xlabel("Number of Category", fontsize=30)
ax[1].invert_xaxis()
ax[1].yaxis.set_label_position("right")
ax[1].yaxis.tick_right()
...
sns.barplot(y="recency", x="customer_city", data=rfm_df.sort_values(by="recency", ascending=False).head(5), palette=colors, ax=ax[0])
ax[0].set_ylabel(None)
ax[0].set_xlabel("customer_city", fontsize=17)
...
ax[1].set_ylabel(None)
ax[1].set_xlabel("customer_city", fontsize=20)
...
ax[2].set_xlabel("customer_city", fontsize=20)
...
st.caption('Copyright (c) Cango56 2023')